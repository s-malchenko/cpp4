...
private:
    ...
    {
    public:
        explicit IndexWrapper(size_t index, Polynomial &poly) : poly_(poly), index_(index)
        {
        }

        operator T() const
        {
            return as_const(poly_)[index_];
        }

        T operator=(T value)
        {
            if (index_ < poly_.coeffs_.size())
            {
                poly_.coeffs_[index_] = value;
                poly_.Shrink();
            }
            else if (value != T(0))
            {
                poly_.coeffs_.resize(index_ + 1);
                poly_.coeffs_[index_] = value;
            }
...
public:
    ...
    IndexWrapper operator [](size_t degree)
    {
        return IndexWrapper(degree, *this);
    }
...
int main()
{
    ...
    RUN_TEST(tr, TestNonconstAccessDouble);
