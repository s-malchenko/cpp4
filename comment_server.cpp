...
public:
    ...
    friend ostream &operator<<(ostream &output, const HttpResponse &resp)
    {
        ...
        for (const auto &i : resp._headers)
        {
            HttpResponse::printHeader(output, i) << endl;
        }

        if (!resp._content.empty())
        {
            HttpResponse::printHeader(output, Header{"Content-Length", to_string(resp._content.size())}) << endl;
            output << endl;
...
private:
    ...
    static ostream &printHeader(ostream &os, const Header &h)
    {
...
{
    stringstream ss;
    ss << HttpResponse(HttpCode::Ok).AddHeader("1", "2").AddHeader("2", "2").SetContent("hello").AddHeader("2", "2");
